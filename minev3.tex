...
\section*{摘~要}
设计有效的奖励信号是强化学习（RL）应用于复杂连续控制和具身智能的核心瓶颈。基于偏好的强化学习（PbRL）通过成对轨迹比较来学习奖励模型，但将其势函数直接应用于基于势的奖励重塑（PBRS）时，常遭遇极端方差和早期梯度崩溃。本文揭示了这一现象的根本原因——\textbf{单调性陷阱（Monotonicity Trap）}：纯排序目标仅能保证轨迹的序数正确性，却无法约束PBRS所依赖的基数增量结构，导致神经网络极易收敛于病态的单调扭曲势函数。为了在不牺牲标签效率的前提下解决这一问题，我们提出两个互补的无监督正则化项：\textbf{最大熵增量先验（$\mathcal{L}_{struct}$）}通过最大化增量分布的熵，将势函数锚定在增量最均匀的结构上；\textbf{单调性铰链损失（$\mathcal{L}_{mono}$）}通过惩罚负增量，确保势函数沿轨迹方向单调递增。二者分别解决增量的均匀性和方向性问题，共同从纯序数偏好中恢复对RL友好的基数增量结构。在LIBERO模拟操作基准测试上的实验表明，我们的方法仅使用廉价的成对轨迹偏好标签，在帧级奖励对齐和轨迹级策略排序上均大幅超越了依赖帧级进度标签的有监督基线（VOC $r$: $0.968$ vs $0.860$，提升$12.6\%$；Kendall $\tau$: $0.696$ vs $0.504$，提升$38.1\%$）。本研究证明，无需昂贵的绝对进度标注，仅凭序数偏好即可恢复稳定且对RL友好的基数增量结构。
...
本文的目标是在不引入这种存在语义对齐风险的绝对进度标签的前提下，弥合偏好学习中的可识别性差距。我们的核心洞察是：恢复对 RL 友好的基数增量结构，只需在纯排序目标上施加基于信息论的轻量级结构约束。具体地，我们引入\textbf{最大熵增量先验}（Maximum Entropy Increment Prior, $\mathcal{L}_{struct}$），通过最大化增量序列分布的熵来鼓励模型在相邻状态转换间均匀分配进度值，并配合\textbf{单调性铰链损失}（$\mathcal{L}_{mono}$）确保势函数沿轨迹方向单调递增。二者共同作用，在无限个满足排序等效的单调函数族中，自动排除导致梯度爆炸或消失的病态扭曲，使势函数网络锚定在增量分布最均匀、方向最一致的结构上。

本文的主要贡献：
\begin{itemize}[leftmargin=*,noitemsep]
    \item \textbf{问题诊断：}我们正式定义并分析了\textbf{单调性陷阱}，通过实验证明纯排序模型虽然能学习到正确的轨迹序数关系，但其帧级增量结构往往发生严重退化，揭示了 PbRL 奖励重塑不稳定的理论根源。
    \item \textbf{方法设计：}我们提出两个互补的结构正则项——$\mathcal{L}_{struct}$将熵最大化原理应用于进度增量分布以保证均匀性，$\mathcal{L}_{mono}$通过铰链损失保证增量方向为正。两者均无需引入环境特权信息或额外的进度标注，即可从序数偏好中恢复出平滑稳定的基数增量结构。
    \item \textbf{实验验证：}在 LIBERO 模拟操作基准 \cite{liu2023libero} 上，我们的方法仅凭成对轨迹偏好，在帧级奖励对齐（VOC $r$: $0.968$ vs $0.860$）和轨迹级策略排序（Kendall $\tau$: $0.696$ vs $0.504$）上均大幅超越了依赖帧级进度监督的有监督基线，有力支持了基数增量结构可以从纯序数信号中恢复的观点。
\end{itemize}
