...
\begin{figure}[htb]
    ...
    \caption{LIBERO-90上三种方法的策略排序指标对比。方法A（纯BT）表现接近随机水平，验证了单调性陷阱的存在。我们的方法C（BT+熵先验）在所有指标上均超越了使用帧级监督的预言机方法D。}
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{我们的方法（C）在所有指标上都超越了监督预言机（D）。}尽管仅使用成对偏好标签（没有帧级进度监督），实验C的VOC $r = 0.968$相比于$0.860$（+12.6\%），Kendall $\tau = 0.696$相比于$0.504$（+38.1\%），排序准确率$= 0.848$相比于$0.752$（+12.8\%），成败差异$= 10.800$相比于$2.175$（+396\%）。这表明$\mathcal{L}_{struct}$配合单调性约束$\mathcal{L}_{mono}$恢复的基数增量结构全面优于监督进度预测。

    \item \textbf{无监督方法在帧级对齐上也超越了监督方法。}实验C的VOC $r$（$0.968$）甚至高于使用帧级进度标签训练的监督预言机（$0.860$），表明$\mathcal{L}_{struct}$和$\mathcal{L}_{mono}$的组合不仅恢复了正确的轨迹级排序结构，还产生了更平滑、更一致的逐帧进度曲线。
...
在SmolVLM-500M完全微调下，配合单调性铰链损失$\mathcal{L}_{mono}$，方向歧义被彻底解决：实验C实现了VOC $r = 0.968$（在LIBERO-90上）和$0.934$（在LIBERO-10上），甚至超过了监督预言机的$0.860$。
...
在LIBERO操作基准测试上的经验评估表明，$\mathcal{L}_{struct}$配合$\mathcal{L}_{mono}$不仅将一个本质上随机的基线（Kendall $\tau \approx 0$）转变为一个强大的策略判别器，而且在包括帧级对齐在内的所有指标上都超越了完全监督的预言机（VOC $r$：$0.968$对$0.860$，Kendall $\tau$：$0.696$对$0.504$，排序准确率：$0.848$对$0.752$）——仅使用成对轨迹偏好，没有帧级进度标签。这一结果验证了我们的核心论点：仅从序数偏好就可以恢复对RL友好的基数结构，而无需监督进度估计方法所需的注释开销。
