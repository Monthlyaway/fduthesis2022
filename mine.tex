...
\begin{table}[ht]
    ...
    \midrule
    A.~纯BT & (待定) & (待定) & (待定) & (待定) \\
    C.~BT+$\mathcal{L}_{struct}$（我们的） & 0.685 & \textbf{0.584} & \textbf{0.792} & \textbf{11.755} \\
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{较低的VOC $r$并不意味着较差的轨迹级奖励质量。}实验C较低的VOC $r$（$0.685$对$0.860$）反映出每帧势函数不如监督进度预测器平滑，这是预期的，因为实验C没有接收帧级标签。然而，显著更高的策略排序分数证实了$\mathcal{L}_{struct}$学习到的基数增量结构产生了更好的轨迹级奖励信号。
...
\begin{figure}[htb]
    ...
    \caption{相同轨迹上实验C（我们的方法，蓝色实线）与实验D（监督预言机，橙色虚线）的进度预测对比。灰色点线为真实时间步索引。实验D的曲线更平滑（更高的VOC $r$），但实验C在轨迹级排序上表现更优，验证了$\mathcal{L}_{struct}$恢复的基数增量结构优于监督进度预测。}
...
在SmolVLM-500M完全微调下，方向歧义没有显现：实验C实现了正的VOC $r$（在LIBERO-90上为$0.685$，在LIBERO-10上为$0.477$），表明完全微调提供了足够的模型容量来学习正确的方向。
