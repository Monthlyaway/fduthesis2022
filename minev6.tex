...
\section*{摘~要}
设计有效的奖励信号是强化学习（RL）应用于复杂连续控制和具身智能的核心瓶颈。基于偏好的强化学习（PbRL）通过成对轨迹比较来学习奖励模型，避免了逐帧标注的高昂代价，但将其势函数直接应用于基于势的奖励重塑（PBRS）时，常遭遇极高的方差和早期梯度崩溃。本文揭示了这一现象的根本原因——\textbf{单调性陷阱（Monotonicity Trap）}：纯排序目标仅能保证轨迹的排序正确性，却无法约束PBRS所依赖的数值增量结构，致使势函数的增量分布发生病态扭曲。由此出发，本文提出核心洞察：恢复对RL友好的数值增量结构无需昂贵的帧级进度标注，只需在排序目标上施加基于信息论的轻量级结构约束。具体地，我们设计了两个互补的无监督正则化项：最大熵增量约束（$\mathcal{L}_{struct}$）通过最大化增量分布的熵来保证均匀性；单调性铰链损失（$\mathcal{L}_{mono}$）通过惩罚负增量来保证方向性。两者协同作用，仅依赖纯排序偏好即可恢复出利于强化学习优化的数值增量结构。在LIBERO模拟操作基准测试上的实验表明，本文方法仅使用低成本的成对轨迹偏好标签，在帧级奖励对齐和轨迹级策略排序上均大幅超越了依赖帧级进度标签的有监督基线（VOC $r$: $0.968$ vs $0.860$，提升$12.6\%$；Kendall $\tau$: $0.696$ vs $0.504$，提升$38.1\%$）。
...
\vspace{1em}
\section*{ABSTRACT}
Designing effective reward signals remains the core bottleneck for applying reinforcement learning (RL) to complex continuous control and embodied intelligence. Preference-based reinforcement learning (PbRL) learns reward models through pairwise trajectory comparisons, bypassing the prohibitive cost of frame-level annotation; however, directly deploying its potential function into potential-based reward shaping (PBRS) often encounters extreme variance and early gradient collapse. This paper reveals the root cause---the \textbf{Monotonicity Trap}: pure ranking objectives can only guarantee ordinal correctness of trajectories but cannot constrain the cardinal increment structure that PBRS relies upon, causing the potential function's increment distribution to become pathologically distorted. Our key insight is that recovering an RL-friendly cardinal increment structure requires no expensive frame-level progress labels, but only lightweight information-theoretic structural constraints on the ranking objective. Concretely, we propose two complementary unsupervised regularizers: a \textbf{Maximum Entropy Increment Constraint ($\mathcal{L}_{struct}$)} that maximizes the entropy of the increment distribution to enforce uniformity, and a \textbf{Monotonicity Hinge Loss ($\mathcal{L}_{mono}$)} that penalizes negative increments to ensure directionality. Together, they recover an RL-friendly cardinal increment structure from pure ordinal preferences. Experiments on the LIBERO simulated manipulation benchmark demonstrate that our method, using only cheap pairwise trajectory preference labels, outperforms fully supervised baselines that rely on frame-level progress labels on both frame-level reward alignment and trajectory-level policy ranking (VOC $r$: $0.968$ vs $0.860$, improved by $12.6\%$; Kendall $\tau$: $0.696$ vs $0.504$, improved by $38.1\%$).
...
然而，将偏好学习到的势函数通过 PBRS 部署时，实践中常出现显著的优化不稳定性乃至早期梯度崩溃 \cite{liang2022rune}。具体而言，即便奖励模型在轨迹排序上近乎完美，下游策略优化仍常因奖励信号的剧烈方差波动而失败。本文揭示了这一现象的根本原因——\textbf{单调性陷阱（Monotonicity Trap）}（\cref{fig:monotonicity_trap}）：排序目标仅保证轨迹间的排序正确性（$\Phi(\tau_w) > \Phi(\tau_l)$），却无法约束 PBRS 所依赖的逐步数值增量（$\Delta\Phi$）。在缺乏结构性约束的情况下，由于神经网络倾向于捷径学习，模型极易收敛，但其学习到的势函数在增量分布上往往呈现出病态的扭曲：虽在轨迹级排序上完全正确，却导致早期探索区域奖励几乎消失、或在任务末期产生巨大的梯度脉冲，破坏了训练稳定性。

\begin{figure}[htb]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/monotonicity-trap.png}
    \caption{\textbf{单调性陷阱示意图。}左图：三种势函数（线性、指数、Sigmoid）在状态排序上完全等价（$\Phi(s_4) > \dots > \Phi(s_1)$），在纯排序损失下不可区分。右图：排序一致性掩盖下的增量结构差异——线性势的增量 $\Delta\Phi$ 均匀分布，可提供稳定引导；指数势在任务末期产生梯度脉冲；Sigmoid势导致早期探索区域奖励稀疏。这种结构性退化是PBRS部署不稳定的根源。}
...
本文致力于在摒弃帧级进度标签依赖的前提下，弥合偏好学习中宏观排序目标与微观数值增量需求之间的鸿沟。核心思路在于：恢复对 RL 友好的数值增量结构，只需在排序目标上施加基于信息论的轻量级结构约束。具体地，我们引入最大熵增量约束（Maximum Entropy Increment Constraint, $\mathcal{L}_{struct}$），通过最大化增量分布的熵来鼓励势函数在相邻状态转换间均匀分配进度值；同时配合单调性铰链损失（$\mathcal{L}_{mono}$）惩罚负增量，确保势函数沿轨迹方向单调递增。两项正则化分别解决增量的均匀性和方向性问题，共同从纯排序偏好中恢复出对 RL 友好的数值增量结构。

本文的主要贡献可概括为以下三个方面。首先，我们定义并分析了\textbf{单调性陷阱}：纯排序模型虽能正确排序轨迹，其帧级增量结构却发生严重退化，揭示了 PbRL 奖励重塑不稳定的根本原因在于排序目标对数值结构的约束不足。其次，为解决这一问题，我们提出两个互补的无监督正则项：$\mathcal{L}_{struct}$通过最大化增量分布的熵保证均匀性，$\mathcal{L}_{mono}$通过铰链损失保证增量方向为正，两者均无需帧级进度标注，即可从排序偏好中恢复出平滑的数值增量结构。最后，在 LIBERO 操作基准 \cite{liu2023libero} 上的实验表明，本文方法仅凭成对轨迹偏好，在帧级奖励对齐（VOC $r$: $0.968$ vs $0.860$，提升$12.6\%$）和轨迹级策略排序（Kendall $\tau$: $0.696$ vs $0.504$，提升$38.1\%$）上均超越了依赖帧级进度监督的有监督基线。
...
然而，上述方法在优化势函数 $\Phi(s)$ 时均依赖于排序损失。从数学本质上看，排序目标仅确保轨迹级的累积增量 $\sum \Delta\Phi$ 满足排序关系，而无法唯一确定瞬时的数值增量结构。正如引言所述，这种目标固有的约束不足使得神经网络极易陷入单调性陷阱：模型可能为了最小化排序损失而走“捷径”，将势函数塌陷为非线性的扭曲形态（如阶跃或指数形状）。这种退化直接导致了基于势的奖励重塑（PBRS）中奖励信号的极高方差。本文的出发点正是通过结构约束弥合排序目标与数值需求之间的鸿沟。
...
传统的解决方案往往应用 $L_2$ 惩罚或简单的时序平滑正则化。我们认为，标准的 $L_2$ 正则化易导致过度平滑，倾向于强制产生僵化的线性插值，这往往会忽略任务中的语义，即在某些关键物理动作完成瞬间（如瓶盖被拧开、夹爪精准触碰目标），奖励应当允许出现合理的非线性跃迁。我们的方法借鉴了信息论中的最大熵原理 \cite{shannon1948, jaynes1957}。不同于 $L_2$ 这种点对点的约束，最大熵增量约束作用于增量的全局概率拓扑。它允许由偏好梯度引导的、位于“语义瓶颈”处的必要奖励峰值涌现，同时确保在未定义的广阔探索区域内，奖励分布能自动平滑至最平坦的状态，从而在稳定训练与保留任务特性之间实现更优的平衡。
...
\end{equation}
经典理论证明，在表格型 MDP 中，该公式保证了最优策略的不变性。在深度强化学习中，尽管由于函数近似和状态分布偏移的存在，这种保证可能受到数值稳定性的挑战，但其在理论上依然提供了将“任务进度约束”转化为“稠密梯度引导”的最优路径 \cite{muller2025improving}。
...
虽然现代 RL 算法如 PPO \cite{schulman2017} 和 SAC \cite{haarnoja2018} 具备 Advantage Normalization 和 Clipping 机制，但这些防御手段主要针对奖励的全局均值和方差进行平滑。当势函数落入单调性陷阱时，其产生的重塑奖励 $F$ 会在关键状态转换处产生极端的“奖励脉冲”。这种局部的数值爆炸会瞬间扭曲信度分配过程，导致梯度估计的方差超出 Clipping 的保护范围，从而引发策略的早期崩溃或陷入局部最优。下一节将提出基于信息论的结构正则化方法，从增量分布的均匀性与方向性两个维度解决这一约束不足问题。
...
如第\ref{sec:pbrs_regularization}节所述，$L_2$时间平滑惩罚是约束增量分布的常见手段，但其逐点回归的刚性本质倾向于将势函数压缩为线性插值，无法区分病态扭曲与任务中固有的语义瓶颈，即关键物理转折点（例如机械臂末端执行器成功抓取目标物体）所应当具备的非线性奖励跃迁。
...
由于增量$\Delta\Phi_t$的取值范围无界，不能直接计算其熵。我们通过Softmax将增量序列映射为概率分布：
\begin{equation}
\label{eq:softmax}
...
$\mathcal{L}_{mono}$作为铰链损失是一种柔性惩罚而非硬约束：它仅对负增量施加线性代价，不约束正增量的大小。该损失对所有训练轨迹统一计算，鼓励势函数整体趋向非递减。对于成功轨迹，这一约束直接反映物理进度的单调递增特性；对于失败轨迹，它作为温和的结构正则化，防止势函数学习到全局递减的退化模式。$\mathcal{L}_{struct}$与$\mathcal{L}_{mono}$在功能上互补：前者约束增量分布的均匀性，后者保证增量的正向单调性。

实现层面，公式中的$\frac{1}{T-1}$归一化系数针对单条轨迹而言；在实际梯度计算中，系统采用基于批次大小（batch size）而非总序列步数的全局归一化策略，以避免不同轨迹长度对梯度缩放产生不一致的影响。
...
从理论动机的角度审视，SAC策略之所以能更快获得有意义的梯度，正是因为$\mathcal{L}_{struct}$与$\mathcal{L}_{mono}$共同打破了引言中提出的单调性陷阱。当势函数落入Sigmoid型退化时，早期探索区域增量趋近于零，PBRS奖励几乎消失；而指数型退化则导致任务末期增量剧烈膨胀，引发梯度脉冲与策略震荡。$\mathcal{L}_{struct}$通过最大化增量分布的熵，从根源上消除这两类极端模式；$\mathcal{L}_{mono}$进一步保证增量方向为正。两者协同，为智能体在探索初期即提供数值稳定且方向正确的稠密引导信号，实现了从理论动机到端到端策略表现的完整闭环。
...
\textbf{局限性与未来工作。}本文的实验验证存在以下局限。首先，所有实验在LIBERO模拟基准的单一平台上进行，下游策略学习实验仅覆盖了一个任务（Task 28），尚未在真实机器人平台和更多样化的任务分布上验证泛化性。其次，$\mathcal{L}_{mono}$假设势函数沿轨迹方向单调递增，这一假设在包含回退、重复尝试等非单调进展模式的任务中可能需要松弛或调整。此外，熵Softmax温度$\alpha$和正则化权重$\lambda$在所有实验中采用固定值，尚未探索任务自适应的超参数调节策略。

基于上述局限，未来工作将沿以下方向展开：（1）将本文方法推广至更多LIBERO任务及真实机器人平台，系统性地验证跨域泛化能力；（2）在更大规模的VLM骨干网络上探索奖励建模能力的可扩展性；（3）研究自适应温度调节与正则化权重调度机制，减少人工超参数调优的需求；（4）探索将最大熵增量约束与在线偏好收集机制结合，实现从人类反馈到策略优化的全流程闭环系统。
