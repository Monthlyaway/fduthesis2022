...
\renewcommand{\mytitle}{基于最大熵增量约束的强化学习奖励重塑方法研究}
\renewcommand{\mytitleEN}{A MAXIMUM ENTROPY INCREMENT CONSTRAINT APPROACH FOR REWARD SHAPING IN REINFORCEMENT LEARNING}
...
\section*{摘~要}
设计有效的奖励信号是将强化学习（RL）扩展到复杂连续控制与具身智能的核心瓶颈。基于偏好的强化学习（PbRL）通过成对轨迹比较习得奖励模型，并借助基于势的奖励重塑（PBRS）部署密集奖励，但将偏好势函数直接应用于PBRS时常遭遇极高方差与梯度崩溃。本文揭示了这一现象的根本原因——\textbf{单调性陷阱（Monotonicity Trap）}：纯排序目标仅保证轨迹排序的正确性，无法约束PBRS所需的逐步数值增量结构，致使势函数的增量分布产生病态扭曲。基于信息论的最大熵原理，我们提出核心洞察：在缺乏任务进度先验时，最优约束应将进度增量尽可能均匀地分配于相邻状态转换中。据此，我们设计了两个互补的无监督正则项——最大熵增量约束保证增量分布的均匀性，单调性铰链损失保证增量的正向单调性——两者协同从纯排序偏好中恢复出对RL友好的数值增量结构，无需帧级进度标注。在LIBERO模拟操作基准上的实验表明，本文方法在帧级奖励对齐（VOC $r$: $0.968$ vs $0.860$，提升$12.6\%$）和轨迹级策略排序（Kendall $\tau$: $0.696$ vs $0.504$，提升$38.1\%$）上均大幅超越使用帧级进度标签的有监督基线。
...
\vspace{1em}
\section*{ABSTRACT}
Designing effective reward signals remains the core bottleneck for applying reinforcement learning (RL) to complex continuous control and embodied intelligence. Preference-based reinforcement learning (PbRL) learns reward models through pairwise trajectory comparisons and deploys dense rewards via potential-based reward shaping (PBRS), yet directly applying preference-learned potential functions to PBRS often suffers from extreme variance and gradient collapse. This paper reveals the root cause---the \textbf{Monotonicity Trap}: pure ranking objectives guarantee only ordinal correctness of trajectories but cannot constrain the cardinal increment structure that PBRS relies upon, causing pathologically distorted increment distributions. Drawing on the maximum entropy principle from information theory, we identify a key insight: in the absence of task progress priors, the optimal increment constraint should distribute progress as uniformly as possible across adjacent state transitions. Accordingly, we introduce two complementary unsupervised regularizers---a \textbf{Maximum Entropy Increment Constraint} that enforces increment uniformity, and a \textbf{Monotonicity Hinge Loss} that ensures positive monotonicity---which together recover an RL-friendly cardinal increment structure from pure ordinal preferences without frame-level progress annotations. Experiments on the LIBERO simulated manipulation benchmark demonstrate that our method outperforms fully supervised baselines relying on frame-level progress labels on both frame-level reward alignment (VOC $r$: $0.968$ vs $0.860$, 12.6\% improvement) and trajectory-level policy ranking (Kendall $\tau$: $0.696$ vs $0.504$, 38.1\% improvement).
...
然而，将偏好学习到的势函数通过 PBRS 部署时，实践中常出现显著的优化不稳定性乃至梯度崩溃 \cite{liang2022rune}：即便奖励模型在轨迹排序上近乎完美，下游策略优化仍因奖励方差过大而失败。本文揭示了这一现象的根本原因——\textbf{单调性陷阱（Monotonicity Trap）}（\cref{fig:monotonicity_trap}）：排序目标仅保证轨迹间的排序正确性（$\Phi(\tau_w) > \Phi(\tau_l)$），却无法约束 PBRS 所依赖的逐步数值增量（$\Delta\Phi$）。由于神经网络倾向于捷径学习，势函数的增量分布极易发生病态扭曲——导致早期探索区域奖励消失、或在任务末期产生剧烈梯度脉冲，破坏训练稳定性。
...
为弥合排序与数值之间的鸿沟，以 ROBOMETER \cite{robometer2026} 为代表的近期过程奖励模型引入了显式进度标签（如时间步比率 $t/T$）约束势函数的数值尺度。然而，这一策略面临语义对齐风险：时间步推进并不等同于任务的真实物理进度，尤其在次优轨迹中存在原地徘徊或失败片段时，$t/T$ 标签会向网络注入矛盾的监督信号。

本文致力于在不依赖帧级进度标签的前提下，弥合排序目标与数值增量需求之间的鸿沟。核心洞察源自最大熵原理 \cite{shannon1948, jaynes1957}：在缺乏任务进度先验时，最优约束策略应将进度增量尽可能均匀地分配于相邻状态转换中。据此，我们提出两个互补的无监督正则项：最大熵增量约束（$\mathcal{L}_{struct}$）通过最大化增量分布的熵保证均匀性，单调性铰链损失（$\mathcal{L}_{mono}$）通过惩罚负增量保证方向正确性。两者在功能上互补——前者约束增量的分布形态，后者约束增量的方向——协同从纯排序偏好中恢复出对 RL 友好的数值增量结构。

本文的主要贡献概括为三点。（1）我们定义并分析了\textbf{单调性陷阱}，揭示了 PbRL 奖励重塑不稳定的根本原因在于排序目标对数值增量结构的约束不足。（2）我们提出最大熵增量约束与单调性铰链损失两个互补的无监督正则项，仅需排序偏好即可恢复平滑的数值增量结构，无需帧级进度标注。（3）在 LIBERO 操作基准 \cite{liu2023libero} 上的实验表明，本文方法在帧级奖励对齐（VOC $r$: $0.968$ vs $0.860$，提升$12.6\%$）和轨迹级策略排序（Kendall $\tau$: $0.696$ vs $0.504$，提升$38.1\%$）上均超越依赖帧级进度监督的有监督基线。
...
然而，上述方法在优化势函数 $\Phi(s)$ 时均依赖于排序损失。从数学本质上看，排序目标仅确保轨迹级的累积增量 $\sum \Delta\Phi$ 满足排序关系，而无法唯一确定瞬时的数值增量结构。这种约束不足使得神经网络极易陷入单调性陷阱：模型为最小化排序损失走捷径，将势函数塌陷为阶跃或指数等扭曲形态，直接导致 PBRS 奖励信号的极高方差。本文正是通过结构约束弥合排序目标与数值需求之间的鸿沟。

\subsection{过程奖励模型与进度监督}

为了缓解奖励稀疏问题，近期研究试图通过估计任务进度来构建密集的过程奖励模型。早期方法主要依赖 VLM 的零样本评分，但由于缺乏时序连贯性，常表现出严重的时序不稳定性，即相邻帧由于视觉微变导致奖励值剧烈震荡。

后续工作如 ROBOMETER \cite{robometer2026} 试图通过显式的进度估计器来平滑信号。这类方法引入绝对进度标签（如 $t/T$）作为强监督。尽管在模拟环境下获取 $t/T$ 成本极低，但在现实机器人任务中，完成任务所需的总步数 $T$ 往往随环境干扰而动态波动，且在无标注的离线演示视频中，$T$ 的界定具有主观性。这种对帧级特权标注的依赖限制了模型向真实场景的迁移。本文方法通过最大熵正则化实现无监督的结构对齐，仅需轨迹级偏好即可恢复平滑进度结构。

\subsection{基于势的奖励重塑及其正则化}
\label{sec:pbrs_regularization}

Ng 等人 \cite{ng1999policy} 提出的基于势的奖励重塑（PBRS）是PbRL中将学习到的势函数转化为重塑奖励的标准范式，其优势在于能够保持最优策略的不变性。然而，在深度 RL 的实际应用中，由于智能体策略不断演化，会引发严重的状态分布偏移，使奖励模型在未见状态下的预测失真并产生高方差梯度 \cite{liang2022rune}。

传统的解决方案往往应用 $L_2$ 惩罚或简单的时序平滑正则化。然而，$L_2$ 正则化倾向于将势函数压缩为僵化的线性插值，忽略了任务中固有的语义非线性——在关键物理动作完成瞬间（如夹爪精准触碰目标），奖励应允许合理的非线性跃迁。

本文方法借鉴信息论中的最大熵原理 \cite{shannon1948, jaynes1957}，从根本上区别于 $L_2$ 的逐点约束。最大熵增量约束作用于增量的全局概率分布：它允许由偏好梯度引导的必要奖励峰值在语义瓶颈处自然涌现，同时确保广阔探索区域内的增量分布自动趋向最均匀的形态，在稳定训练与保留任务特性之间实现更优的平衡。

\section{预备知识}

\subsection{面向具身智能的马尔可夫决策过程}

我们将具身智能中的机器人操作任务建模为连续空间的马尔可夫决策过程（MDP），定义为元组 $\mathcal{M} = \langle \mathcal{S}, \mathcal{A}, \mathcal{P}, r, \gamma \rangle$ \cite{sutton2018}。状态空间 $\mathcal{S}$ 由视觉观测与本体感知信息组成，动作空间 $\mathcal{A} \in \mathbb{R}^d$ 为连续的关节指令空间。$\mathcal{P}(s'|s,a)$ 为环境动力学，$\gamma \in [0, 1)$ 为折扣因子。
...
\end{equation}
经典理论证明，该公式在表格型 MDP 中保证最优策略的不变性 \cite{muller2025improving}。在深度 RL 中，函数近似和状态分布偏移可能削弱这一保证，但 PBRS 仍是将任务进度约束转化为稠密梯度引导的标准路径。
...
这一线性叠加假设赋予了奖励模型马尔可夫属性，使下游 RL 优化在数学上可行。模型通过最小化交叉熵损失进行训练：
\begin{equation}
\label{eq:bt}
...
现代 RL 算法如 PPO \cite{schulman2017} 和 SAC \cite{haarnoja2018} 虽具备 Advantage Normalization 和 Clipping 机制，但这些防御手段仅针对全局方差进行平滑，无法应对单调性陷阱导致的局部数值爆炸。当势函数的增量分布严重扭曲时，重塑奖励 $F$ 在关键转换处产生的极端脉冲会超出 Clipping 的保护范围，引发策略崩溃。下一节将提出针对这一问题的结构正则化方法。

\section{方法}

本节提出一种在离线偏好学习阶段直接约束势函数增量结构的方法，在不引入绝对进度标签的前提下利用信息论结构约束防止增量退化。下文依次介绍最大熵增量约束（保证均匀性）、单调性铰链损失（保证方向性）、联合优化目标，以及下游 RL 部署方式。训练好的势函数网络在未见任务上直接泛化，无需重新训练或微调。

\subsection{最大熵增量约束}

如第\ref{sec:pbrs_regularization}节所述，$L_2$时间平滑惩罚是约束增量分布的常见手段，但其逐点回归的刚性本质倾向于将势函数压缩为线性插值，无法区分病态扭曲与关键物理转折点处应有的非线性奖励跃迁。为在防止退化的同时保留灵活的进度分配，我们基于最大熵原理\cite{shannon1948,jaynes1957}转向对增量序列的全局概率结构施加约束：在缺乏任务进度先验时，最优策略应将进度尽可能均匀地分配于相邻状态转换中。
...
$\mathcal{L}_{struct}$通过最大化增量分布的熵来鼓励进度均匀分配，但Softmax熵具有方向不变性，无法区分"所有增量均匀为正"与"所有增量均匀为负"。实验表明，在缺乏方向约束时，模型倾向于学习均匀递减的势函数（详见第\ref{sec:direction}节），尽管其满足熵最大化目标，却导致PBRS奖励信号方向反转。为此，我们引入单调性铰链损失：
\begin{equation}
\label{eq:lmono}
    \mathcal{L}_{mono} = \mathbb{E}_{\tau \sim \mathcal{D}} \left[ \frac{1}{T-1}\sum_{t=0}^{T-2} \max(0, -\Delta\Phi_t) \right]
\end{equation}

$\mathcal{L}_{mono}$作为铰链损失是一种柔性惩罚而非硬约束：它仅对负增量施加线性代价，不约束正增量的大小。该损失对所有训练轨迹统一计算，鼓励势函数整体趋向非递减。对于成功轨迹，这一约束直接反映物理进度的单调递增特性；对于失败轨迹，它作为温和的结构约束，避免势函数学习到全局递减的退化模式。$\mathcal{L}_{struct}$与$\mathcal{L}_{mono}$在功能上互补：前者约束增量分布的均匀性，后者保证增量的正向单调性。
...
\end{equation}
其中$\lambda_{struct}$和$\lambda_{mono}$分别控制均匀性正则化和方向约束的强度。三项损失在功能上互补：$\mathcal{L}_{BT}$确保轨迹间的排序正确，$\mathcal{L}_{struct}$规范增量的均匀分布，$\mathcal{L}_{mono}$保证增量方向为正。所有实验取$\lambda_{struct} = 0.1$、$\lambda_{mono} = 0.1$。训练动态实验（\cref{tab:dynamics}）证实三项损失在训练过程中同步收敛、互不干扰。超参数的设置见第\ref{sec:training_dynamics}节。

\subsection{下游RL部署与泛化}

离线训练收敛后，势函数网络$\Phi_\theta$的参数冻结，直接在未见的目标任务上进行泛化评估。在PBRS框架下，智能体在每步接收稠密奖励：
\begin{equation}
\label{eq:deploy}
    F(s_t, a_t, s_{t+1}) = \gamma \Phi_\theta(s_{t+1}) - \Phi_\theta(s_t)
\end{equation}
PBRS的策略不变性保证了重塑奖励不会引入奖励作弊。本文实验在LIBERO-90（未见任务集）上评估奖励模型的帧级对齐和轨迹级排序质量，验证结构正则化带来的增量改善能否跨任务泛化。第\ref{sec:policy_learning}节进一步在单一任务上验证改善后的奖励信号能否驱动下游策略学习。
...
前三种方法共享相同的模型架构：每帧势函数$\Phi_\theta$（无Sigmoid激活的进度头），帧上求和得到轨迹级排序分数$\Phi(\tau) = \sum_{t} \Phi_\theta(s_t)$。三者仅在损失函数上有所区别：纯BT基线不加正则化；BT+L2平滑以$\mathcal{L}_{L2} = \sum_t (\Delta\Phi_t - \overline{\Delta\Phi})^2$鼓励增量均匀；本文方法使用$\mathcal{L}_{struct}$配合$\mathcal{L}_{mono}$。有监督基线采用ROBOMETER原始架构，包含独立的偏好头和帧级监督进度头（$t/T$标签），在监督水平上构成上界参照。L2正则化权重$\lambda = 0.1$，本文方法超参数见第\ref{sec:joint_objective}节，温度$\alpha = 0.1$。
...
\textbf{本文方法在帧级对齐上大幅领先。}VOC $r = 0.968$远高于纯BT基线（$0.147$）、L2平滑（$0.616$）和有监督基线（$0.860$）。仅使用成对偏好标签，$\mathcal{L}_{struct}$配合$\mathcal{L}_{mono}$恢复的数值增量结构甚至优于帧级监督，表明信息论结构约束在增量质量上的有效性可以超越显式的逐帧监督信号。

\textbf{单调性陷阱得到实验验证。}纯BT基线的轨迹级排序尚可（Kendall $\tau = 0.680$，排序准确率$= 0.840$），但VOC $r$仅$0.147$，这意味着势函数虽然能够正确排序整条轨迹，却产生不了合理的逐帧进度结构。这正是\cref{fig:monotonicity_trap} 所描述的现象：排序正确性与增量结构质量完全脱耦，纯排序目标在微观数值层面的约束力几乎为零。

\textbf{L2平滑不足以解决问题。}L2平滑将VOC $r$从$0.147$提升至$0.616$，说明时间平滑确实有助于缓解增量方差，但其改善仍远不充分。根本原因在于，$L_2$惩罚仅约束增量幅度的均匀性，而不施加方向约束，增量完全可以均匀地为负（详见第\ref{sec:direction}节的分析）。

\textbf{排序能力与增量结构的质量独立变化。}三种BT方法的轨迹级排序几乎相同（Kendall $\tau$：$0.680$--$0.696$），但VOC $r$从$0.147$阶梯式上升到$0.968$。这直接验证了本文的核心论点：正则化策略决定增量结构的质量，与排序能力本身无关。值得注意的是，纯BT基线和L2平滑的成败差异略高于本文方法（$14.9$/$15.1$ vs $10.8$），这是$\mathcal{L}_{mono}$使增量更均匀分布的预期代价，即以少量成败分离度换取了大幅提升的帧级对齐质量。
...
移除Sigmoid激活后，势函数的单调方向不再有保证，模型可能沿成功轨迹递减而非递增。这种方向歧义不影响轨迹级排序（BT损失对$\Phi$的全局符号不敏感），但会导致PBRS奖励信号方向反转。

$\mathcal{L}_{mono} = \text{mean}(\text{ReLU}(-\Delta\Phi_t))$通过惩罚负增量来解决这一问题。加入$\mathcal{L}_{mono}$后，本文方法在LIBERO-90上达到VOC $r = 0.968$，在LIBERO-10上达到$0.934$，均超过有监督基线的$0.860$。作为对照，不使用方向约束的纯BT基线VOC $r$仅为$0.147$，L2平滑为$0.616$，可见方向约束对帧级对齐至关重要。
...
选取LIBERO-90基准中的Task 28（"close the top drawer of the cabinet"，关闭柜子顶部抽屉）作为测试任务，与ROBOMETER\cite{robometer2026}原文实验保持一致。策略算法采用Soft Actor-Critic（SAC）\cite{haarnoja2018}，使用DINOv2\cite{oquab2024dinov2} ViT-S/14提取384维视觉特征，拼接9维本体感受向量（关节角度 + 夹爪状态）构成393维的观测空间，输入两层256维MLP构成的Actor和Critic网络。

奖励设计方面，比较两种方案：
\begin{itemize}[leftmargin=*,noitemsep]
    \item \textbf{BT+MaxEnt（本文方法）}：使用本文训练的奖励模型通过PBRS框架提供密集奖励信号。每一步的奖励为$r_t = \gamma\Phi(s_{t+1}) - \Phi(s_t)$，其中$\Phi$为奖励模型预测的进度势函数。
    \item \textbf{Sparse Reward（稀疏基线）}：仅使用环境原生的稀疏奖励，即每步$-1$，任务成功时$0$。不加载VLM奖励模型。
\end{itemize}

主要超参数：折扣因子$\gamma = 0.99$，批大小256，学习率$3 \times 10^{-4}$，目标网络EMA系数$\tau = 0.005$，自动调节熵系数$\alpha$。每5000步在25条评估回合上测量成功率。

\subsubsection{RL结果分析}
...
从理论角度看，策略性能的提升源于$\mathcal{L}_{struct}$与$\mathcal{L}_{mono}$对单调性陷阱的有效抑制。$\mathcal{L}_{struct}$将进度值均匀分配于整条轨迹，消除了Sigmoid型退化（早期奖励消失）和指数型退化（末期梯度爆炸）两类极端模式；$\mathcal{L}_{mono}$保证增量方向为正，确保重塑奖励与任务推进方向一致。两者协同为智能体在探索初期即提供数值稳定的稠密引导信号，实现了从离线奖励建模到端到端策略表现的完整闭环。
...
本文揭示了基于偏好的奖励模型中一个此前被忽视的结构性问题，即单调性陷阱：纯排序目标虽然能保证排序正确性，但无法约束PBRS部署所需的数值增量结构。为此，我们提出了最大熵增量约束（$\mathcal{L}_{struct}$），一种轻量的无监督正则化项，通过最大化时间增量分布的熵来规范势函数的增量结构，配合单调性铰链损失$\mathcal{L}_{mono}$确保增量方向正确。
...
\textbf{局限性。}本文存在以下局限：（1）实验评估仅在LIBERO模拟基准上进行，尚未在真实机器人平台上验证泛化性；（2）所有实验基于SmolVLM-500M单一视觉语言模型骨架，更大规模VLM的表现有待考察；（3）下游RL验证仅覆盖单一任务，多任务场景下的策略学习效果尚需系统评估。

\textbf{未来工作。}在此基础上，我们规划了三个后续方向：（1）将方法推广至完整的LIBERO-90任务集和真实机器人平台，验证结构正则化在物理环境中的鲁棒性；（2）探索更大规模VLM（如2B--7B参数量级）作为奖励模型骨架，考察模型容量对增量结构恢复质量的影响；（3）将最大熵增量约束与在线偏好采集相结合，实现人在回路的持续奖励优化。
...
感谢吴祖煊老师在研究方向和论文写作上的悉心指导。感谢ROBOMETER\cite{robometer2026}开源代码库。本研究使用的计算资源由AutoDL云平台提供。
